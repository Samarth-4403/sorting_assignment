...
# Extract page numbers using map and split, then sort based on these numbers
page_numbers = list(map(lambda x: int(x.split('_')[1].split('.')[0]), file_paths))
sorted_file_paths = [path for _, path in sorted(zip(page_numbers, file_paths))]
